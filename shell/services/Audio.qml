    ...
    function refreshNodes(): void {
        ...
        for (const node of Pipewire.nodes.values) {
            if (!node.isStream) {
                ...
                const name = (node.properties?.["application.name"] || node.name || node.description || "").toLowerCase();
                const appId = (node.properties?.["application.id"] || "").toLowerCase();
                if (!name.includes("caelestia") && !name.includes("quickshell") && !appId.includes("caelestia") && !appId.includes("quickshell")) {
